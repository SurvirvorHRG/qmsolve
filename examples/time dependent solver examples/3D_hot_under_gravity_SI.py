...
visualization.plot_hot(t=0,L_norm = w_o,Z_norm = lambda_,unit= 1e-3,dis= dis)
visualization.plot_hot(t=total_time,L_norm = w_o,Z_norm = lambda_,unit= 1e-3,dis= dis)
visualization.final_plot3D_hot(L_norm = w_o,Z_norm = lambda_,unit= 1e-3,g=g,tmax=total_time)
#for i in range(129):
    #visualization.plot2D_hot_xz(t=i * total_time/128,L_norm = w_o,Z_norm = lambda_,unit=seconds * 1e-3)

for i in range(21):
    visualization.plot_hot(t=i * total_time/20,L_norm = w_o,Z_norm = lambda_,unit= 1e-3,g=g,dis= dis)
